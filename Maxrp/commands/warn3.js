        ...
        try {
            ...
            for (let a in config.warnAdministrationList) {
                if (message.member.roles.cache.has(config.warnAdministrationList[a])) {
                    message.delete();
                    message.reply(`Gratulacje, pomyślnie nadałeś warn #3 **obywatelelowi** ${mentioned} 🎉`).then(msg => msg.delete({timeout: 5000}));
                    ...
                    const warnEmbedMessageToChannel = new Discord.MessageEmbed()
                        ...
                        .setColor(0x00FF00)
                        .setAuthor('WARN ROOM', '','')
                        .setFooter(config.serverName + "MaxRP 2.0 ■ Witter")
                        .setThumbnail("https://media0.giphy.com/media/3Z1frUjlKRIqJhnhAW/source.gif");
                    mentioned.roles.add(config.warnRoleID3);
